...
import { ENV_CONFIG, SERVER_CONFIG, isDevelopment, isProduction } from './config/env';
import { DatabaseConnectionStatus } from './types/api';
...
class ServerManager {
  ...
  private validateEnvironment(): void {
    ...
    if (ENV_CONFIG.PORT < 1 || ENV_CONFIG.PORT > 65535) {
      throw new Error(`Invalid port number: ${ENV_CONFIG.PORT}`);
    }

    // Production-specific validations
    if (isProduction()) {
      if (ENV_CONFIG.JWT_SECRET === 'default-jwt-secret-change-in-production') {
  ...
  private configureServer(): void {
    ...
    this.server.on('error', (error: any) => {
      if (error.code === 'EADDRINUSE') {
        console.error(`❌ Port ${ENV_CONFIG.PORT} is already in use`);
        process.exit(1);
      } else if (error.code === 'EACCES') {
        console.error(`❌ Permission denied to bind to port ${ENV_CONFIG.PORT}`);
        process.exit(1);
      } else {
  ...
  private async listen(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.server.listen(ENV_CONFIG.PORT, ENV_CONFIG.HOST, () => {
        console.log(`🌐 Server listening on ${ENV_CONFIG.HOST}:${ENV_CONFIG.PORT}`);
        console.log(`🔗 Health check: http://${ENV_CONFIG.HOST}:${ENV_CONFIG.PORT}/health`);
        console.log(`📡 API endpoint: http://${ENV_CONFIG.HOST}:${ENV_CONFIG.PORT}/api/v1`);
        
        if (isDevelopment()) {
          console.log(`📖 API docs: http://${ENV_CONFIG.HOST}:${ENV_CONFIG.PORT}/api/v1/docs`);
        }
  ...
  private async performHealthChecks(): Promise<void> {
    ...
    try {
      ...
      if (ENV_CONFIG.NANOBANANA_API_KEY) {
        console.log('✅ External API configuration: OK');
      } else {
        console.warn('⚠️  External API configuration: Missing API key');
      }
  ...
  private setupGracefulShutdown(): void {
    const signals = SERVER_CONFIG.gracefulShutdown.signals;
  ...
  private async shutdown(exitCode: number = 0): Promise<void> {
    ...
    const shutdownTimeout = setTimeout(() => {
      ...
      process.exit(1);
    }, SERVER_CONFIG.gracefulShutdown.timeout);
